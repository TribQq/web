...
export class BbDetailComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.bbservice.getBbs(pk).subscribe((bb: Object) => {
      this.bb = bb;
      this.getComments();
    });
  }
...
// 680 Служба Acti vatedRoute позволяет получить сведения об интернет-адресе, по ко­
// торому был выполнен переход, включая значения присутствующих в нем URL­ параметров
